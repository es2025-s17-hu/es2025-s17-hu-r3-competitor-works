 ...
 */
const EditMenuItemModal = ({ open, onClose, item, setItems, categories }) => {
  const { token } = useAuth();
  ...
  const [menuCategoryId, setMenuCategoryId] = useState(item.menuCategoryId);
  ...
  async function handleSave() {
    ...
    setItems((prev) =>
      prev.map((x) =>
        x.id === item.id ? { ...x, name, price, type, menuCategoryId } : x
      )
    );
    onClose();
  }
  ...
  async function handleDelete() {
    axios.delete(`/menuItems/${item.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    });
